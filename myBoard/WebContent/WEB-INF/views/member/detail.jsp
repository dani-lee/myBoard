					...
					<div class="row">
						...
						<div class="col-sm-3 text-center">
							<c:if test="${member.enabled ne 0}">
								<button type="button" onclick="location.href='enabled?id=${member.id}&enabled=0';" id="stopBtn" class="btn btn-btn-info">비활성</button>
							</c:if>
							<c:if test="${member.enabled eq 0}">
								<button type="button" onclick="location.href='enabled?id=${member.id}&enabled=1';" id="activeBtn" class="btn btn-btn-info">활&nbsp;&nbsp;성</button>
							</c:if>
						</div>
						<%-- </c:if> --%>
