					...
					<form role="form" class="form-horizontal" action="regist.do" method="post">						
						...
						<div class="input-group mb-3">
							<div class="mailbox-attachments clearfix" style="text-align: center;">
								<div class="mailbox-attachment-icon has-img" id="pictureView" style="border: 1px solid green; height: 200px; width: 140px; margin: 0 auto; overflow:hidden;"></div>
								<div class="mailbox-attachment-info">
									<div class="input-group input-group-sm">
										<label for="inputFile" class=" btn btn-primary btn-sm btn-flat input-group-addon" >파일선택</label>
										<input type="file" id="inputFile" style="display:none;" onchange="imgPreview(event);" />
...
//  let fileName = fileValue[fileValue.length-1]; // 파일명
    let fileName = document.getElementById("inputFile").files[0].name;  // 파일명
...
function imgPreview(event) {
    ...
    $("div#pictureView").empty();

        let img = document.createElement("img");
        img.setAttribute("src", event.target.result);
        img.setAttribute("id", "newImg");
        // img.setAttribute("style", "overflow:hidden");
        document.querySelector("div#pictureView").appendChild(img);
    };
